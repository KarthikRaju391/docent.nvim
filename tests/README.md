...
- `minimal_init.lua` — editor init: prepend repo to `runtimepath`, call
  `require('docent').setup()` (editors run with `--noplugin -i NONE`).
...
| `handshake` | `initialize` → `protocolVersion`, `capabilities.tools`, `serverInfo.name == "docent"`, nonempty `instructions` with navigation guidance; `notifications/initialized` gets no response; `tools/list` returns exactly the 7 tools each with `inputSchema.type == "object"`; unknown method → JSON-RPC error `-32601`; stdout contained only valid JSON lines throughout. |
...
| `tour` | First `add_tour_stop` auto-jumps; stops 2/3 do not move the cursor; `list_tour` mentions all three stop files; `]t` advances to stop 2 then 3; `]t` past the end does not move; `clear_tour` empties `list_tour`. |
